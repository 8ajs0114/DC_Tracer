...
interrupt void ADC_TIMER_ISR(void)
{
	...
	l_adc_sum_left += (long)AdcMirror.ADCRESULT0;
	l_adc_sum_left += (long)AdcMirror.ADCRESULT1;
	l_adc_sum_left += (long)AdcMirror.ADCRESULT2;
	l_adc_sum_left += (long)AdcMirror.ADCRESULT3;	
	//about AdcRegs.ADCCHSELSEQ1
	
	l_adc_sum_right += (long)AdcMirror.ADCRESULT4;
	l_adc_sum_right += (long)AdcMirror.ADCRESULT5;
	l_adc_sum_right += (long)AdcMirror.ADCRESULT6;
	l_adc_sum_right += (long)AdcMirror.ADCRESULT7;
	//about AdcRegs.ADCCHSELSEQ2
	
	l_adc_sum_left += (long)AdcMirror.ADCRESULT8;
	l_adc_sum_left += (long)AdcMirror.ADCRESULT9;
	l_adc_sum_left += (long)AdcMirror.ADCRESULT10;
	l_adc_sum_left += (long)AdcMirror.ADCRESULT11; 
	//about AdcRegs.ADCCHSELSEQ3
	
	l_adc_sum_right += (long)AdcMirror.ADCRESULT12;
	l_adc_sum_right += (long)AdcMirror.ADCRESULT13;
	l_adc_sum_right += (long)AdcMirror.ADCRESULT14;
	l_adc_sum_right+= (long)AdcMirror.ADCRESULT15;
	...
	g_sen[g_u32_sen_cnt].iq15_4095_value = l_adc_sum_left << 12; // 여러번 받은 값 평균 내는 중 . 형 변환중 사라지는거 생각 
	g_sen[SEN_NUM + g_u32_sen_cnt ].iq15_4095_value = l_adc_sum_right << 12;	 //divide into 8(2^3)(R:15+L:14=R:1)
	...
	//already reseted "g_u32_copmare_cnt" intro "0" in main.c
...
void make_position(position_t *p_pos, sen_t *p_sen)
{
	...
	if( p_pos->iq15_sum )
	{
		...
		if( p_pos->iq7_temp_pos >= POS_END )		
			p_pos->iq7_temp_pos = POS_END;

		else if( p_pos->iq7_temp_pos <= -POS_END )		
			p_pos->iq7_temp_pos = -POS_END;
